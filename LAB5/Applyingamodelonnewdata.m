...
fs = 250;
t = (0:length(n_423)-1)/fs;


normal_seg = n_423(1:1+fs*10);
abnormal_seg = n_423(11442 : 11442 + fs*10);
...
num_windows = floor((length(n_423) - window_length)/step) + 1;
...
for i = 1:num_windows
    ...
    if end_idx > length(n_423)
        break;
    end

    window = n_423(start_idx:end_idx);
...
[alarm_n423_h,t] = va_detect_424(n_423,fs, valid_indices);

valid_indices_2 = [find(true_labels==1) ; find(true_labels == 2)];
GT=GT_h(valid_indices_2);
alarm_n423=alarm_n423_h(valid_indices_2);
%% 2.7 
ground_truth = GT;
predicted = alarm_n423;
